...
import { getRealtimeService, RealtimeService, VenueUpdate, VibeCheckUpdate, PromotionUpdate, NotificationData } from './realtime-service'

// Hook for managing realtime connection
export function useRealtime(wsUrl: string, userId?: string) {
  const [isConnected, setIsConnected] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const serviceRef = useRef<RealtimeService | null>(null)

  useEffect(() => {
    const service = getRealtimeService(wsUrl, userId)
...
export function useVenueUpdates(service: RealtimeService | null, callback: (update: VenueUpdate) => void) {
  useEffect(() => {
    if (!service) return
...
export function useVenueSpecific(service: RealtimeService | null, venueId: string, callback: (update: VenueUpdate) => void) {
  useEffect(() => {
...
export function useVibeChecks(service: RealtimeService | null, venueId: string, callback: (vibeCheck: VibeCheckUpdate) => void) {
  useEffect(() => {
    if (!service || !venueId) return
...
// Hook for promotions
export function usePromotions(service: RealtimeService | null, venueId: string, callback: (promotion: PromotionUpdate) => void) {
  useEffect(() => {
    if (!service || !venueId) return
...
export function useNotifications(service: RealtimeService | null, callback: (notification: NotificationData) => void) {
  useEffect(() => {
    if (!service) return
...
export function useRealtimeConnection(wsUrl: string, userId?: string) {
