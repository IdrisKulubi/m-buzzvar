...
export function useAuthRole() {
      ...
      if (session?.user && !isPending) {
        try {
          ...
          if (response.ok) {
            ...
            console.warn("Failed to fetch role, using default");
            setRole("user"); // Default role
          }
        } catch (error) {
          console.error("Error fetching user role:", error);
          console.warn("Using default role due to error");
          setRole("user"); // Default role
        }
  ...
  return {
    ...
    isVenueOwner:
      role === "venue_owner" || role === "admin" || role === "super_admin",
...
export async function hasPermission(permission: string): Promise<boolean> {
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      return (
        data.permissions.includes(permission) || data.permissions.includes("*")
      );
    }
    return false;
  } catch (error) {
